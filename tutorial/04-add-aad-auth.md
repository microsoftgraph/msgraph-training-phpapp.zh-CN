...
1. 在 PHP 应用程序的根目录中打开该 **env** 文件，并将以下代码添加到该文件的末尾。

    :::code language="ini" source="../demo/graph-tutorial/.env.example" id="OAuthSettingsSnippet":::

1. 将替换 `YOUR_APP_ID_HERE` 为应用程序注册门户中的应用程序 ID，并 `YOUR_APP_PASSWORD_HERE` 将替换为您生成的密码。

    > [!IMPORTANT]
    > 如果您使用的是源代码管理（如 git），现在可以 `.env` 从源代码管理中排除该文件，以避免无意中泄漏您的应用程序 ID 和密码。
...
1. 在 **/app/Http/Controllers** 目录中创建一个名为的新文件 `AuthController.php` ，并添加以下代码。
    ...
    这将定义包含两个操作的控制器： `signin` 和 `callback` 。

    该 `signin` 操作将生成 AZURE AD 登录 URL，保存 `state` OAuth 客户端生成的值，然后将浏览器重定向到 Azure AD 登录页。
...
1. 启动服务器并浏览到 `https://localhost:8000` 。 单击 "登录" 按钮，您应会被重定向到 `https://login.microsoftonline.com` 。 使用你的 Microsoft 帐户登录。

1. 检查同意提示。 权限列表与在 **. env**中配置的权限范围列表相对应。
...
1. 将 `try` 方法中的块替换 `callback` 为以下代码。
...
1. 在 **/app** 目录中创建一个名为的新目录 `TokenStore` ，然后在名为的该目录中创建一个新文件， `TokenCache.php` 并添加以下代码。
...
1. 将 `try` 现有函数中的块替换 `callback` 为以下项。
